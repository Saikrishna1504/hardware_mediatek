...
typedef enum {
    WIFI_LSTATS_SUBCMD_GET_INFO = ANDROID_NL80211_SUBCMD_LSTATS_RANGE_START,
} WIFI_LLS_SUB_COMMAND;
...
} WIFI_USABLE_CHANNEL_REQ_ATTRIBUTE;

typedef enum {
    WIFI_ATTRIBUTE_USABLE_CHANNEL_RESP_INVALID,
    WIFI_ATTRIBUTE_USABLE_CHANNEL_RESP_ARRAY,
    WIFI_ATTRIBUTE_USABLE_CHANNEL_RESP_MAX
} WIFI_USABLE_CHANNEL_RESP_ATTRIBUTE;

typedef struct {
    uint16_t array_size;
    wifi_usable_channel channel_array[];
} wifi_usable_channel_response;
...
typedef struct {
    ...
    void* cb_arg;
    char ifname[MAX_IFACENAME_LEN + 1];  // initialized when register vendor handler,
                                         // not used in wifi_register_handler
} cb_info;
...
typedef struct {
    ...
    char name[MAX_IFACENAME_LEN + 1];  // interface name + trailing null
